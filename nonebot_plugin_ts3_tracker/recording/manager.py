...
from ..channels import channel_matches_token, resolve_channel_tokens
...
from .slice import (
    RECORDER_HEADER_BYTES,
    SliceError,
    SliceResult,
    slice_wav_tail,
    write_slice_metadata,
)
from .users import channel_human_stats

RECORDING_DISABLED_MESSAGE = (
...
class RecordingManager:
    ...
    async def sync(
        self, status: Ts3ServerStatus, *, now: datetime | None = None
    ) -> None:
        ...
        async with self._lock:
            ...
            for channel_id in list(self._sessions.keys()):
                ...
                if resolved:
                    ...
                    session.participant_names.update(participant_names)
                else:
            ...
            for channel_id in sorted(monitored.keys()):
                if channel_id in self._sessions:
                    continue
                self._pending_stops.pop(channel_id, None)
                human_count, _ = channel_human_stats(self.settings, status, channel_id)
    ...
    async def slice_active_sessions(
        ...
        *,
        ...
        if channel_filter is not None:
            sessions, error = self._filter_sessions_by_channel(
                sessions,
        ...
        disambiguate_channel = output_basename is not None and len(sessions) > 1
        ...
        for session in sessions:
            try:
                wav_path, metadata_path = build_slice_paths(
                    ...
                    duration_minutes,
                    output_basename,
                    disambiguate_channel=disambiguate_channel,
                )
            except ValueError as exc:
                errors.append(f"{session.channel_name} ({session.channel_id})：{exc}")
                continue
            try:
    ...
    async def force_start_sessions(
        ...
        *,
        ...
        if not monitored:
            return [], [
                "未解析到可录制的监控频道，请检查 TS3_TRACKER__RECORDING_CHANNELS。"
            ]
        ...
        if channel_filter is not None:
            targets = {
                ...
                if channel_matches_token(
                    channel_id=channel_id,
                    channel_name=channel_name,
                    token=channel_filter,
                )
            }
            if not targets:
                configured = ", ".join(
    ...
    def _filter_sessions_by_channel(
        self,
        sessions: list[ChannelRecordingSession],
        *,
        channel_filter: str,
        active_sessions: list[ChannelRecordingSession] | None = None,
    ) -> tuple[list[ChannelRecordingSession], str | None]:
        filtered = [
            session
            for session in sessions
            if channel_matches_token(
        ...
        active = active_sessions if active_sessions is not None else sessions
        active_text = ", ".join(
            f"{item.channel_name}({item.channel_id})" for item in active
        )
        return [], (
            f"未找到匹配的进行中录音频道：{channel_filter}。当前录音：{active_text}"
        )
    ...
    async def _start_session(
        ...
        *,
        ...
        output_dir = resolve_recordings_dir(self.settings)
        ...
        session = ChannelRecordingSession(
            ...
            nickname=nickname,
            participant_names=channel_human_stats(self.settings, status, channel_id)[1],
        )
    ...
    async def _stop_session(self, channel_id: str, *, ended_at: datetime) -> None:
        ...
        if duration < self.settings.recording_min_session_seconds:
            ...
            await asyncio.to_thread(self._discard_session_files, session)
            logger.error(
                "TS3 recorder produced no usable audio for channel {} ({}) "
                "with exit code {}",
                session.channel_id,
                session.channel_name,
                exit_code,
            ...
            log = logger.info if exit_code in (0, None) else logger.warning
            log(
    ...
    @staticmethod
    def _has_audio_file(path: Path) -> bool:
        try:
            return path.is_file() and path.stat().st_size > RECORDER_HEADER_BYTES
        except OSError:
            return False
